...
def execute():
    ...
    if(likability>0.5567):
        likability=1
    else:
        likability=0
    #likabilityData=JSONEncoder().encode({"likability":likability})
    mqttc.publish("$aws/things/eeg/shadow/update", likability, qos=0)
    tencount=tencount+1
    if(tencount==10):
        mqttc.publish("$aws/things/eeg/shadow/update",likability,qos=0)
        tencount=0
...
mqttc.tls_set(caPath, certfile=certPath,
              keyfile=keyPath, cert_reqs=ssl.CERT_REQUIRED, tls_version=ssl.PROTOCOL_TLSv1_2, ciphers=None)
    ...
    if state == 0:
        ...
        if ready == 1:
            ...
            if connflag == True:
                ...
                insertData=JSONEncoder().encode({"Valence":(((alphaArr[0]/betaArr[0])-(alphaArr[1]/betaArr[1]))-1)})
                if(alphaAvg!=-1):
